...
% This basic version is for a sphere of uniform temperature
%***********************************************************************
...
while (time < tmax*3600.)
  ...
  k = find(p_s==0);  % all molecules residing on surface
  p_t(k) = residence_time(Tsurf);  % relative to time
  ...
  [p_r,p_s,p_t,p_n,ccc] = montecarlo(NP,p_r,p_s,p_t,p_n,Tsurf,dtsec,ccc,Q);
...
disp(sprintf('# particles destroyed, photo %d',ccc(1)))
