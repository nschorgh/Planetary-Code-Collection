...
subroutine deriv2_coeff(z,nz,c)
  ! coefficients for second derivative (a*b_z)_z on irregular grid
  ! 2nd order corner-free
  implicit none
  integer, intent(IN) :: nz
  real*8, intent(IN) :: z(nz)
  real*8, intent(OUT) :: c(3,nz)
  integer j
  real*8 hm,hp
  ...
  do j=2,nz-1
     hp=z(j+1)-z(j)
     hm=z(j)-z(j-1)
     c(1,j)=1./(hm*(z(j+1)-z(j-1)))
     c(2,j)=1./(hp*hm)
     c(3,j)=1./(hp*(z(j+1)-z(j-1)))
  enddo

end subroutine deriv2_coeff
