...
C***********************************************************************
      ...
      parameter (NMAX=2000, sigSB=5.67051d-8)
...
!      call smartgrid(nz,z,0.d0,thermalInertia,rhoc,0.4d0,ti,rhocv)
!      call smartgrid_allice(nz,z,0.5d0,thermalInertia,rhoc,ti,rhocv)
...
!      write(21,'(f12.6,2f10.4)') time,marsLs/d2r,Th+T(1)
...
!         dE = (- Qn - Qnp1 + Fsurfold + Fsurf +
!     &        emiss*sigSB*(Tsurfold**4+Tsurf**4))/2.
!         print *,Qnp1,Fsurf,emiss*sigSB*Tsurf**4,dE
...
c--------only output and diagnostics below this line
         if (time>=tmax-solsy) then
            Tmean=Tmean+Tsurf
            Tmean2=Tmean2+T(nz)
            ps = ps + min(psv(Tsurf),0.1202d0)/Tsurf
            pb = pb + psv(T(nz))/T(nz)
            nm=nm+1
         endif
         if (time>=tmax-solsy.and.mod(n,max(1,nsteps/100000))==0) then
