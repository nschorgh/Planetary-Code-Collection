...
C   
C   written by Norbert Schorghofer, ~2004
C***********************************************************************
      ...
      real*8 psv, avdrho, rtbis, junk, Tb
      external psv, rtbis

C-------set input parameters
      dt=0.01
      nz=80; zfac=1.05;
      fracIR=0.04; fracDust=0.0
      !Fgeotherm=0.028
      Fgeotherm=0.0
      icefrac = 0.4
...
c      open(unit=34,file='means',status='unknown')
...
C        Empirical relation from Mellon & Jakosky:
     ...
     &           fracDust,Fgeotherm,dt,zfac,icefrac,slp,azFac)
...
C     finds root with bisection method a la Numerical Recipes (C)
      ...
      integer nz
c      fmid=func(x2,
c     &     latitude,albedo0,thIn,pfrost,nz,fracIR,fracDust,Fgeotherm,
c     &     dt,zfac,icefrac)
c      print *,x2,fmid
      Tb = -1.e32
      call jsub(x1,
     &     latitude,albedo0,thIn,pfrost,nz/2,rhoc,fracIR,fracDust,
     &     Fgeotherm,4.*dt,zfac,icefrac,surfaceSlope,azFac,1,f,Tb)
      call jsub(x1,
     &     latitude,albedo0,thIn,pfrost,nz,rhoc,fracIR,fracDust,
     &     Fgeotherm,dt,zfac,icefrac,surfaceSlope,azFac,0,f,Tb)
      ...
      do j=1,JMAX
        ...
        call jsub(xmid,
     &     latitude,albedo0,thIn,pfrost,nz/2,rhoc,fracIR,fracDust,
     &     Fgeotherm,4.*dt,zfac,icefrac,surfaceSlope,azFac,1,fmid,Tb)
        call jsub(xmid,
     &     latitude,albedo0,thIn,pfrost,nz,rhoc,fracIR,fracDust,
