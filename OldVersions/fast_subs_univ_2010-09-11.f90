...
pure function colint(y,z,nz,i1,i2)
  ...
  integer i
  real(8) dz(nz)
  dz(1)=z(1) ! this leaves z(1)/2 for surface point
  do i=2,nz-1
     dz(i) = (z(i+1)-z(i-1))/2.
  enddo
  dz(nz) = z(nz)-z(nz-1)
...
function equildepth(nz, z, rhosatav, rhosatav0, avrho1)
!***********************************************************************
...
!-fill depth
  ...
  do i=1,nlb
     ...
     leak = leak  ! avoids compiler warning
     !help(i) = help(i)-leak
     if (help(i) <= 0.) then
        typeF=i
...
subroutine icechanges_poreonly(nz,z,typeF,typeG,avdrhoP,ypp,B,porefill)
  use allinterfaces
  implicit none
...
subroutine icechanges(nz,z,typeF,avdrho,avdrhoP,ypp, &
...
!***********************************************************
  ...
  if (zdepthT>=0. .and. avdrho>0.) then
     beta = 2*Diff*bigstep*(1-icefrac)/(1-porosity)*86400*365.24/icedensity
     zdepthT = sqrt(beta*avdrho*18./8314. + zdepthT**2)
  endif
